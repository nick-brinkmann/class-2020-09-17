...
```{r}
raw <- readxl::read_xlsx(path = "raw_data/class_enrollment_summary_by_term_9-17-2020.xlsx", skip = 3)
...
en_fall_2020 %>%
  slice(1:10) %>%
  ggplot(aes(x = fct_reorder(course_title, u_grad), y = u_grad)) +
  geom_col() +
  coord_flip() +
  labs(title = "Largest Undergraduate Harvard Courses", x = "", y = "Undergraduate Enrollment")
```
